...
function teamQuality(team: Team): number {
  const sorted = [...team.players].sort((a, b) => b.rating - a.rating).slice(0, 11);
...
function pickScorer(ctx: Ctx, side: Side): { player: Player; assist?: Player } | null {
  ...
  const weights = atk.map((p) => Math.pow(p.rating, 2));
  ...
  if (mid.length > 0 && Math.random() < 0.5) {
    const w2 = mid.map((p) => Math.pow(p.rating, 1.5));
...
function maybeInjury(ctx: Ctx): MatchEvent | null {
  ...
  const pl = pick(outfieldPlayers(team));
  if (!pl) return null;
  const severities = ["minor", "minor", "moderate", "severe"] as ("minor" | "moderate" | "severe")[];
